...
def run_noisy_inference_for_tq_circuits_qiskit(circ_dir, num_qubits, meas_qubits, device_name,
    ...
    classes_of_interest = [0, 1, 8, 9]

    # FMNIST test (10k) → custom test ds
    fmnist_test = FashionMNIST(root = "./fmnistdata_test", train = False, download = True)
    test_imgs = fmnist_test.data.numpy()
    test_lbls = fmnist_test.targets.numpy()

    mask_test = np.isin(test_lbls, classes_of_interest)
    test_imgs_f = test_imgs[mask_test]
    label_map = {digit: idx for idx, digit in enumerate(classes_of_interest)}
    test_lbls_f = np.array([label_map[l] for l in test_lbls[mask_test]])

    test_ds = AugmentedZigzagDataset(imgs = test_imgs_f, lbls = test_lbls_f)
    print(len(test_ds))
...
num_shots = 10000
best_t = 0.5
circs_dir = '/home/singhg12/isilon/Gurinder/Benchmark_medmnist/QCS_gpu_binary_scale/git/trained_models/fmnist4/trained_circuit/'
num_test_samples = 4000
transpiler_opt_level = 3
results_save_dir = '/home/singhg12/isilon/Gurinder/Benchmark_medmnist/QCS_gpu_binary_scale/git/trained_models/fmnist4/trained_circuit/inference_sim'
